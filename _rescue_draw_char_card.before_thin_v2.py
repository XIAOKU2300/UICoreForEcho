...
from ..utils.api.wwapi import get_rank_url, OneRankRequest, OneRankResponse
...
# EchoMatrix 配色常量(本次仅提取,值暂保持原样;后续调色只改这里)
TEXT_MAIN = (248, 250, 252)                # 主文字色 EchoMatrix 正白 #F8FAFC
TEXT_SUB = (148, 163, 184, 255)           # 副文字色 #94A3B8(保留alpha)
TEMPLATE_NAME_COLOR = (0, 229, 255)       # 模板名青蓝 #00E5FF
BADGE_BG_DARK = (0, 0, 0, int(0.8 * 255)) # 声骸等级徽章背景(半透明黑)
BADGE_BG_SCORE = (30, 41, 59, int(0.8 * 255))   # 分数徽章深空蓝灰 #1E293B(保留0.8透明)
PANEL_BG_70 = (0, 0, 0, int(0.7 * 255))   # 权重表整体背景
PANEL_BG_90 = (0, 0, 0, 90)               # 权重表标题/偶数行背景
PANEL_BG_110 = (0, 0, 0, 110)             # 评分规则面板背景
ROW_BG_ODD = (255, 255, 255, 30)          # 权重表奇数行背景
DIVIDER_45 = (255, 255, 255, 45)          # 分割线(评分规则)
DIVIDER_80 = (255, 255, 255, 80)          # 分割线(最优卡)

# --- EchoMatrix v2 design tokens ---
BG_MAIN_V2 = (11, 13, 18, 255)               # v2 主背景 #0B0D12
BG_CARD_V2 = (17, 20, 28, 255)               # v2 卡片背景 #11141C
ACCENT_SILVER = (203, 213, 225, 255)          # v2 银白 #CBD5E1
ACCENT_CYAN = (0, 229, 255, 255)              # v2 青蓝 #00E5FF
    ...
    if role_detail.phantomData and role_detail.phantomData.equipPhantomList:
        ...
        for i, _phantom in enumerate(equipPhantomList):
            ...
            if _phantom and _phantom.phantomProp:
                ...
                ph_level_img_draw.rounded_rectangle([0, 0, 84, 30], radius=8, fill=BADGE_BG_DARK)
                draw_text_with_fallback(ph_level_img_draw, (8, 13), f"Lv.{_phantom.level}", TEXT_MAIN, waves_font_24, "lm")
                ...
                ph_score_img_draw.rounded_rectangle([0, 0, _score_w, 30], radius=8, fill=BADGE_BG_SCORE)
                draw_text_with_fallback(ph_score_img_draw, (_score_w // 2, 13), f"{_score}{t('分', locale)}", TEXT_MAIN, waves_font_24, "mm")
        ...
        if phantom_score > 0:
            ...
            draw_text_with_fallback(score_temp_draw, (180, 380), f"{phantom_score:.2f}{t('分', locale)}", TEXT_MAIN, waves_font_40, "mm")
            ...
            score_temp_draw = ImageDraw.Draw(score_temp)
            draw_text_with_fallback(score_temp_draw, (180, 130), t("暂无", locale), TEXT_MAIN, waves_font_40, "mm")
            draw_text_with_fallback(score_temp_draw, (180, 380), f"- {t('分', locale)}", TEXT_MAIN, waves_font_40, "mm")
        ...
        draw_text_with_fallback(ph_tips_draw, (20, 50), t("评分模板", locale), TEXT_MAIN, waves_font_24, "lm")
        draw_text_with_fallback(ph_tips_draw, (350, 50), t(calc.calc_temp['name'], locale, partial=True), TEMPLATE_NAME_COLOR, waves_font_24, "rm")
...
async def draw_fixed_img(img, avatar, account_info, role_detail, locale="", uid=None, char_name=None, user_pref=""):
    ...
    draw_text_with_fallback(base_info_draw, (275, 120), f"{_name}", TEXT_MAIN, waves_font_30, "lm")
    ...
    if account_info and account_info.is_full:
        ...
        draw_text_with_fallback(title_bar_draw, (510, 78), f"Lv.{account_info.level}", TEXT_MAIN, waves_font_42, "mm")

        draw_text_with_fallback(title_bar_draw, (660, 125), t("索拉等阶", locale), GREY, _level_font, "mm")
        draw_text_with_fallback(title_bar_draw, (660, 78), f"Lv.{account_info.worldLevel}", TEXT_MAIN, waves_font_42, "mm")
...
async def draw_char_detail_img(
    ...
    if damage_calc and damageDetail and role_detail.phantomData and role_detail.phantomData.equipPhantomList:
        ...
        draw_text_with_fallback(damage_bar_draw, (400, 50), t(damage_title, locale, partial=True), TEXT_MAIN, waves_font_24, "rm")
        if crit_damage and expected_damage:
            draw_text_with_fallback(damage_bar_draw, (700, 50), f"{crit_damage}", TEXT_MAIN, waves_font_24, "mm")
            draw_text_with_fallback(damage_bar_draw, (1000, 50), f"{expected_damage}", TEXT_MAIN, waves_font_24, "mm")
        else:
            draw_text_with_fallback(damage_bar_draw, (850, 50), f"{expected_damage}", TEXT_MAIN, waves_font_24, "mm")
        damage_calc_img.alpha_composite(damage_bar, dest=(0, 70))

        damage_title_bg = damage_bar1.copy()
        damage_title_bg_draw = ImageDraw.Draw(damage_title_bg)
        draw_text_with_fallback(damage_title_bg_draw, (600, 50), t("buff列表", locale), TEXT_MAIN, waves_font_24, "mm")
        ...
        for dindex, effect in enumerate(damageAttributeTemp.effect):
            ...
            damage_bar_draw = ImageDraw.Draw(damage_bar)
            draw_text_with_fallback(damage_bar_draw, (400, 50), t(buff_name, locale, partial=True), TEXT_MAIN, waves_font_24, "rm")
            draw_text_with_fallback(damage_bar_draw, (800, 50), f"{buff_value}", TEXT_MAIN, waves_font_24, "mm")
    ...
    draw_text_with_fallback(weapon_bg_temp_draw, (203, weapon_name_y + 45), f"Lv.{weaponData.level}/90", TEXT_MAIN, waves_font_30, "lm")
    ...
    draw_text_with_fallback(weapon_bg_temp_draw, (_x, _y), f"{t('精', locale)}{weaponData.resonLevel}", TEXT_MAIN, waves_font_24, "lm")
    ...
    if skill_branch:
        draw_text_with_fallback(weapon_bg_temp_draw, (115, weapon_bg_y + 207), _ws0_name, TEXT_MAIN, waves_font_30, "lm")
        draw_text_with_fallback(weapon_bg_temp_draw, (115, weapon_bg_y + 257), _ws1_name, TEXT_MAIN, waves_font_30, "lm")
        draw_text_with_fallback(weapon_bg_temp_draw, (115 + 300, weapon_bg_y + 207), f"{weapon_detail.stats[0]['value']}", TEXT_MAIN, waves_font_30, "rm")
        draw_text_with_fallback(weapon_bg_temp_draw, (115 + 300, weapon_bg_y + 257), f"{weapon_detail.stats[1]['value']}", TEXT_MAIN, waves_font_30, "rm")
        ...
        draw_text_with_fallback(weapon_bg_temp_draw, (130, weapon_bg_y + 207), _ws0_name, TEXT_MAIN, waves_font_30, "lm")
        draw_text_with_fallback(weapon_bg_temp_draw, (130, weapon_bg_y + 257), _ws1_name, TEXT_MAIN, waves_font_30, "lm")
        draw_text_with_fallback(weapon_bg_temp_draw, (500, weapon_bg_y + 207), f"{weapon_detail.stats[0]['value']}", TEXT_MAIN, waves_font_30, "rm")
        draw_text_with_fallback(weapon_bg_temp_draw, (500, weapon_bg_y + 257), f"{weapon_detail.stats[1]['value']}", TEXT_MAIN, waves_font_30, "rm")
    ...
    for i, _mz in enumerate(role_detail.chainList):
        ...
        if locale == 'en' and len(name) > 14 and ' ' in name:
            ...
            name = name[:split_pos] + "\n" + name[split_pos + 1:]
        if len(name) >= 8:
            draw_text_with_fallback(mz_bg_temp_draw, (147, 230), name, TEXT_MAIN, waves_font_16, "mm")
        else:
            draw_text_with_fallback(mz_bg_temp_draw, (147, 230), name, TEXT_MAIN, waves_font_20, "mm")
    ...
    for _, _skill in enumerate(role_detail.get_skill_list()):
        ...
        draw_text_with_fallback(skill_bg_draw, (150, 83), t(_skill.skill.type, locale), TEXT_MAIN, _skill_font, "lm")
        draw_text_with_fallback(skill_bg_draw, (150, 113), f"Lv.{_skill.level}", TEXT_MAIN, waves_font_25, "lm")
    ...
    if score_report is not None:
        ...
        draw_text_with_fallback(score_draw, (400, 1140 + bar_shift), f"{score_report.score:.2f}", TEXT_MAIN, waves_font_40, "mm")
...
def _render_optimal_phantom_card(slot) -> Image.Image:
    ...
    draw_text_with_fallback(draw, (10, y_main), slot.main1_name, TEXT_MAIN, waves_font_22, "lm")
    ...
    draw_text_with_fallback(draw, (10, y_main2), slot.main2_name, TEXT_MAIN, waves_font_22, "lm")
    draw_text_with_fallback(draw, (340, y_main2), main2_val_str, SPECIAL_GOLD, waves_font_22, "rm")

    # 分隔线
    draw.line([(10, y_main2 + 25), (340, y_main2 + 25)], fill=DIVIDER_80, width=1)
    ...
    for si, (sname, sval) in enumerate(slot.subs[:5]):
        ...
        draw_text_with_fallback(draw, (10, ys), sname, TEXT_SUB, waves_font_20, "lm")
        draw_text_with_fallback(draw, (340, ys), sub_val_str, TEXT_MAIN, waves_font_20, "rm")
...
async def ph_card_draw_optimal(
    ...
    async def _draw_best_card(i, slot):
        ...
        tpl_badge_draw.rounded_rectangle([0, 0, _tpl_w, 30], radius=8, fill=BADGE_BG_DARK)
        draw_text_with_fallback(tpl_badge_draw, (_tpl_w // 2, 15), f"Lv.25 {t('模板声骸', locale)}", TEXT_MAIN, waves_font_18, "mm")
    ...
    draw_text_with_fallback(score_temp_draw, (180, 380), f"150.00{t('分', locale)}", TEXT_MAIN, waves_font_40, "mm")
...
async def draw_char_optimize_img(ev: Event, uid: str, char: str, user_id: str, waves_id: Optional[str] = None, change_list_regex: Optional[str] = None, is_limit_query=False):
    ...
    for dindex, (left, right) in enumerate(score_rows):
        damage_bar = damage_bar2.copy() if dindex % 2 == 0 else damage_bar1.copy()
        damage_bar_draw = ImageDraw.Draw(damage_bar)
        draw_text_with_fallback(damage_bar_draw, (300, 50), left, TEXT_MAIN, waves_font_24, "rm")
        draw_text_with_fallback(damage_bar_draw, (700, 50), right, TEXT_MAIN, waves_font_24, "mm")
    ...
    rp_draw.rounded_rectangle(
        [0, 0, _rule_w - 1, rule_panel_h - 1], radius=20, fill=PANEL_BG_110
    )
    draw_text_with_fallback(
    ...
    for idx, row in enumerate(rows[:8]):
        ...
        draw_text_with_fallback(sh_bg_draw, (120, y + 18), name_text, TEXT_MAIN, waves_font_24, "lm")
        draw_text_with_fallback(
            sh_bg_draw, (480, y + 18),
            val_text,
            TEXT_MAIN, waves_font_20, "rm",
        )
        if row["delta_str"]:
            draw_text_with_fallback(
                sh_bg_draw, (578, y + 18),
...
def draw_weight(image, role_name, weight_list_temp, calc_temp, modal_name=""):
    ...
    draw.rectangle([10, 10, 1490, 870], fill=PANEL_BG_70)
    ...
    for i, row in enumerate(weight_list_temp):
        for j, cell in enumerate(row.split(",")):
            ...
            # 绘制单元格背景
            if i == 0:  # 标题行
                draw.rectangle([x, y, x + cell_width, y + cell_height], fill=PANEL_BG_90)
            elif i % 2 == 1:  # 奇数行
                draw.rectangle([x, y, x + cell_width, y + cell_height], fill=ROW_BG_ODD)
            else:  # 偶数行
                draw.rectangle([x, y, x + cell_width, y + cell_height], fill=PANEL_BG_90)
    ...
    draw_text_with_fallback(draw, (start_x, 750), text, font=waves_font_24, fill=TEXT_MAIN)
    s = calc_temp["total_grade"]
    text = f"声骸评分标准：SSS≥{s[-1] * 250:.2f}分/ SS≥{s[-2] * 250:.2f}分／S≥{s[-3] * 250:.2f}分 / A≥{s[-4] * 250:.2f}分 / B≥{s[-5] * 250:.2f}分 / C"
    draw_text_with_fallback(draw, (start_x, 800), text, font=waves_font_24, fill=TEXT_MAIN)
    text = "当前角色评分标准仅供参考与娱乐，不代表任何官方或权威的评价。"
    draw_text_with_fallback(draw, (start_x, 850), text, font=waves_font_24, fill=TEXT_MAIN)
...
# ============================================================
# EchoMatrix v2 renderer (skeleton + top info bar)
# ============================================================

async def draw_char_detail_img_v2(
    ev: Event,
    uid: str,
    char: str,
    user_id,
    ...
    show_score=True,
    fallback_to_generic=False,
    # v2 optional data overrides (for preview / testing)
    _account_info: Optional["AccountBaseInfo"] = None,
    _role_detail: Optional["RoleDetailData"] = None,
    _char_name: Optional[str] = None,
    _avatar_img=None,
    _props=None,  # list[(name, value_str)] override for preview
):
    """EchoMatrix v2 character detail card renderer."""
    # --- resolve display data ---
    char_name = _char_name or char
    account = _account_info
    role_detail = _role_detail

    player_name = getattr(account, "name", None) or char_name or "---"
    player_uid = str(getattr(account, "id", None) or uid or "--------")
    player_level = getattr(account, "level", None)
    world_level = getattr(account, "worldLevel", None)

    # --- canvas ---
    W, H = 1200, 1420
    # --- background: frosted glass with readability mask ---
    try:
        img = await get_card_bg(W, H, "bg3")
        img = img.convert("RGBA")
        if img.size != (W, H):
            img = img.resize((W, H), Image.Resampling.LANCZOS)
    except Exception:
        img = Image.new("RGBA", (W, H), BG_MAIN_V2)
    _bg_mask = Image.new("RGBA", (W, H), (11, 13, 18, 160))
    img = Image.alpha_composite(img, _bg_mask)
    draw = ImageDraw.Draw(img)

    # ============================================================
    # Top info bar  (Y 40 - 120, full width)
    # ============================================================

    # --- avatar circle (center 72,72  r=32) ---
    avatar_cx, avatar_cy, avatar_r = 72, 72, 32
    avatar_bbox = (avatar_cx - avatar_r, avatar_cy - avatar_r,
                   avatar_cx + avatar_r, avatar_cy + avatar_r)
    if _avatar_img is not None:
        av = _avatar_img.resize((avatar_r * 2, avatar_r * 2), Image.Resampling.LANCZOS)
        mask = Image.new("L", (avatar_r * 2, avatar_r * 2), 0)
        ImageDraw.Draw(mask).ellipse((0, 0, avatar_r * 2 - 1, avatar_r * 2 - 1), fill=255)
        img.paste(av, (avatar_cx - avatar_r, avatar_cy - avatar_r), mask)
    else:
        draw.ellipse(avatar_bbox, fill=(40, 44, 54, 255), outline=ACCENT_SILVER, width=2)

    # --- player name  (x=120, y=45) ---
    draw_text_with_fallback(
        draw, (120, 45), player_name,
        fill=TEXT_MAIN, font=youshe_font_28, anchor="lt",
    )

    # --- UID / feature code  (x=120, y=80) ---
    uid_display = "UID " + player_uid
    draw_text_with_fallback(
        draw, (120, 80), uid_display,
        fill=ACCENT_CYAN, font=iosevka_font_18, anchor="lt",
    )
    ...
    # 联觉等级
    draw_text_with_fallback(
        draw, (_cx1, _label_y), "\u8054\u89c9\u7b49\u7ea7",
        fill=TEXT_SUB, font=misans_font_18, anchor="mt",
    )
    lv_text = "Lv." + str(player_level) if player_level is not None else "Lv.--"
    draw_text_with_fallback(
        draw, (_cx1, _value_y), lv_text,
        fill=ACCENT_SILVER, font=iosevka_font_24, anchor="mt",
    )

    # 索拉等阶
    draw_text_with_fallback(
        draw, (_cx2, _label_y), "\u7d22\u62c9\u7b49\u9636",
        fill=TEXT_SUB, font=misans_font_18, anchor="mt",
    )
    wl_text = str(world_level) if world_level is not None else "--"
    draw_text_with_fallback(
        draw, (_cx2, _value_y), wl_text,
        fill=ACCENT_CYAN, font=iosevka_font_24, anchor="mt",
    )

    # --- decorative separator between the two center groups ---
    _sep_x = (_cx1 + _cx2) // 2  # midpoint between the two groups
    draw.line([(_sep_x, _label_y), (_sep_x, _value_y + 8)], fill=(*ACCENT_CYAN[:3], 80), width=1)

    # --- right: "EchoMatrix" brand  (x=1160, right-aligned) ---
    draw_text_with_fallback(
        draw, (1160, 60), "EchoMatrix",
        fill=ACCENT_SILVER, font=youshe_font_32, anchor="rm",
    )

    # ============================================================
    # Middle column: attribute panel  (x 516-830, y 140+)
    # ============================================================
    _mc_x = 516
    _mc_w = 314
    _mc_top = 140
    ...
    # --- resolve props list ---
    attr_props = _props  # caller-supplied list of (name, value_str)
    if attr_props is None:
        attr_props = []
        if role_detail is not None:
            add_list = getattr(role_detail, "equipPhantomAddPropList", None)
            if add_list:
                for p_item in add_list:
                    nm = getattr(p_item, "attributeName", None) or ""
                    vl = getattr(p_item, "attributeValue", None) or ""
                    if nm:
                        attr_props.append((nm, str(vl)))
    ...
    _title_y = 168
    _card_bottom_pad = 24
    _mc_height = (_row_start_y - _mc_top) + len(attr_props) * _row_h + _card_bottom_pad

    _card_r = 12
    _card_fill = BG_CARD_V2
    _card_border = (51, 65, 85)
    _card_xy = (_mc_x, _mc_top, _mc_x + _mc_w, _mc_top + _mc_height)
    try:
        draw.rounded_rectangle(_card_xy, radius=_card_r, fill=_card_fill, outline=_card_border, width=1)
    except AttributeError:
        draw.rectangle(_card_xy, fill=_card_fill, outline=_card_border, width=1)

    _title_x = _mc_x + 16
    draw_text_with_fallback(
        draw, (_title_x, _title_y), "\u5c5e\u6027",
        fill=ACCENT_SILVER, font=misans_font_20, anchor="lm",
    )
    _line_y = _title_y + 14
    draw.line([(_title_x, _line_y), (_title_x + 28, _line_y)], fill=ACCENT_CYAN, width=2)

    _label_x = _mc_x + 16
    _value_x = _mc_x + 298
    for _i, (_attr_name, _attr_val) in enumerate(attr_props):
        _ry = _row_start_y + _i * _row_h
        draw_text_with_fallback(
            draw, (_label_x, _ry), _attr_name,
            fill=TEXT_SUB, font=misans_font_18, anchor="lm",
        )
        draw_text_with_fallback(
            draw, (_value_x, _ry), _attr_val,
            fill=ACCENT_SILVER, font=iosevka_font_18, anchor="rm",
        )
        if _i % 2 == 0:
            _row_bg = (255, 255, 255, 8)
            _row_rect = (_mc_x + 1, _ry - 16, _mc_x + _mc_w - 1, _ry + 16)
            draw.rectangle(_row_rect, fill=_row_bg)

    # ============================================================
    # Right column: weapon card  (x 846-1160, y 140+)
    # ============================================================
    _rc_x = 846
    _rc_w = 314
    _rc_top = 140
    ...
    if role_detail is not None:
        ...
        if wd:
            ...
            _wpn_name = getattr(wpn, "weaponName", None) or "---"
            _wpn_level = getattr(wd, "level", None)
            _wpn_reson = getattr(wd, "resonLevel", None)
            _wpn_atk = getattr(wpn, "weaponAtk", None)
            _wpn_stat_name = getattr(wpn, "weaponStatName", None)
            _wpn_stat_val = getattr(wpn, "weaponStatVal", None)

    _wpn_row_start = 260
    _wpn_row_h = 30
    _wpn_title_y = 168
    _wpn_card_bottom_pad = 24
    _wpn_rows = 5
    _rc_height = (_wpn_row_start - _rc_top) + _wpn_rows * _wpn_row_h + _wpn_card_bottom_pad

    _rc_card_xy = (_rc_x, _rc_top, _rc_x + _rc_w, _rc_top + _rc_height)
    try:
        draw.rounded_rectangle(_rc_card_xy, radius=12, fill=BG_CARD_V2, outline=(51, 65, 85), width=1)
    except AttributeError:
        draw.rectangle(_rc_card_xy, fill=BG_CARD_V2, outline=(51, 65, 85), width=1)

    _wpn_title_x = _rc_x + 16
    draw_text_with_fallback(
        draw, (_wpn_title_x, _wpn_title_y), "\u6b66\u5668",
        fill=ACCENT_SILVER, font=misans_font_20, anchor="lm",
    )
    _wpn_line_y = _wpn_title_y + 14
    draw.line([(_wpn_title_x, _wpn_line_y), (_wpn_title_x + 28, _wpn_line_y)], fill=ACCENT_CYAN, width=2)

    _wpn_icon_x = _rc_x + 16
    _wpn_icon_y = 195
    _wpn_icon_sz = 50
    _wpn_icon_xy = (_wpn_icon_x, _wpn_icon_y, _wpn_icon_x + _wpn_icon_sz, _wpn_icon_y + _wpn_icon_sz)
    draw.rectangle(_wpn_icon_xy, fill=(40, 44, 54, 160), outline=(51, 65, 85, 100), width=1)

    draw_text_with_fallback(
        draw, (_wpn_icon_x + _wpn_icon_sz + 12, _wpn_icon_y + _wpn_icon_sz // 2), _wpn_name,
        fill=ACCENT_SILVER, font=misans_font_20, anchor="lm",
    )

    _wpn_label_x = _rc_x + 16
    _wpn_val_x = _rc_x + 298
    _wpn_stat_y1 = _wpn_icon_y + _wpn_icon_sz + 12
    _wpn_stat_y2 = _wpn_stat_y1 + 32
    _wpn_stat_y3 = _wpn_stat_y2 + 32
    _wpn_stat_y4 = _wpn_stat_y3 + 32

    draw_text_with_fallback(draw, (_wpn_label_x, _wpn_stat_y1), "\u7b49\u7ea7", fill=TEXT_SUB, font=misans_font_18, anchor="lm")
    _lv_str = "Lv." + str(_wpn_level) if _wpn_level is not None else "Lv.--"
    draw_text_with_fallback(draw, (_wpn_val_x, _wpn_stat_y1), _lv_str, fill=ACCENT_SILVER, font=iosevka_font_18, anchor="rm")

    draw_text_with_fallback(draw, (_wpn_label_x, _wpn_stat_y2), "\u5171\u9e23", fill=TEXT_SUB, font=misans_font_18, anchor="lm")
    _res_str = "R" + str(_wpn_reson) if _wpn_reson is not None else "R-"
    draw_text_with_fallback(draw, (_wpn_val_x, _wpn_stat_y2), _res_str, fill=ACCENT_SILVER, font=iosevka_font_18, anchor="rm")

    draw_text_with_fallback(draw, (_wpn_label_x, _wpn_stat_y3), "\u653b\u51fb\u529b", fill=TEXT_SUB, font=misans_font_18, anchor="lm")
    _atk_str = str(_wpn_atk) if _wpn_atk is not None else "--"
    draw_text_with_fallback(draw, (_wpn_val_x, _wpn_stat_y3), _atk_str, fill=ACCENT_SILVER, font=iosevka_font_18, anchor="rm")

    draw_text_with_fallback(draw, (_wpn_label_x, _wpn_stat_y4), _wpn_stat_name or "\u526f\u5c5e\u6027", fill=TEXT_SUB, font=misans_font_18, anchor="lm")
    _stat_str = str(_wpn_stat_val) if _wpn_stat_val is not None else "--"
    draw_text_with_fallback(draw, (_wpn_val_x, _wpn_stat_y4), _stat_str, fill=ACCENT_SILVER, font=iosevka_font_18, anchor="rm")

    # ============================================================
    # Right column: skill card  (x 846-1160, below weapon)
    # ============================================================
    _sk_top = _rc_top + _rc_height + 16
    _sk_title_y = _sk_top + 28

    _skills = []
    if role_detail is not None:
        _skills = getattr(role_detail, "skillList", None) or []

    _sk_row_h = 32
    _sk_row_start = _sk_top + 60
    _sk_card_bottom_pad = 24
    _sk_height = 60 + len(_skills) * _sk_row_h + _sk_card_bottom_pad

    _sk_card_xy = (_rc_x, _sk_top, _rc_x + _rc_w, _sk_top + _sk_height)
    try:
        draw.rounded_rectangle(_sk_card_xy, radius=12, fill=BG_CARD_V2, outline=(51, 65, 85), width=1)
    except AttributeError:
        draw.rectangle(_sk_card_xy, fill=BG_CARD_V2, outline=(51, 65, 85), width=1)

    _sk_title_x = _rc_x + 16
    draw_text_with_fallback(
        draw, (_sk_title_x, _sk_title_y), "\u6280\u80fd",
        fill=ACCENT_SILVER, font=misans_font_20, anchor="lm",
    )
    _sk_line_y = _sk_title_y + 14
    draw.line([(_sk_title_x, _sk_line_y), (_sk_title_x + 28, _sk_line_y)], fill=ACCENT_CYAN, width=2)

    _sk_label_x = _rc_x + 16
    _sk_val_x = _rc_x + 298
    for _si, _sk in enumerate(_skills):
        _sy = _sk_row_start + _si * _sk_row_h
        _sk_obj = getattr(_sk, "skill", None)
        _sk_name = getattr(_sk_obj, "name", None) or "---"
        _sk_type = getattr(_sk_obj, "type", None) or ""
        _sk_lv = getattr(_sk, "level", None)

        draw_text_with_fallback(
            draw, (_sk_label_x, _sy), _sk_type,
            fill=TEXT_SUB, font=misans_font_14, anchor="lm",
        )
        _sk_lv_text = "Lv." + str(_sk_lv) if _sk_lv is not None else "Lv.--"
        draw_text_with_fallback(
            draw, (_sk_val_x, _sy), _sk_lv_text,
            fill=ACCENT_CYAN, font=iosevka_font_18, anchor="rm",
        )

    # ============================================================
    # Right column: evaluation bar  (x 846-1160, below skills)
    # ============================================================
    _rb_top = _sk_top + _sk_height + 16
    _rb_title_y = _rb_top + 28

    _rb_score = None
    _rb_grade = None
    _rb_eval_color = ACCENT_SILVER
    try:
        if role_detail is not None:
            _rb_score, _rb_grade = calc_phantom_score(role_detail)
            if _rb_grade:
                _rb_eval_color = ACCENT_CYAN
    except Exception:
        pass

    _rb_height = 100
    _rb_card_xy = (_rc_x, _rb_top, _rc_x + _rc_w, _rb_top + _rb_height)
    try:
        draw.rounded_rectangle(_rb_card_xy, radius=12, fill=BG_CARD_V2, outline=(51, 65, 85), width=1)
    except AttributeError:
        draw.rectangle(_rb_card_xy, fill=BG_CARD_V2, outline=(51, 65, 85), width=1)

    _rb_title_x = _rc_x + 16
    draw_text_with_fallback(
        draw, (_rb_title_x, _rb_title_y), "\u8bc4\u5206",
        fill=ACCENT_SILVER, font=misans_font_20, anchor="lm",
    )
    _rb_line_y = _rb_title_y + 14
    draw.line([(_rb_title_x, _rb_line_y), (_rb_title_x + 28, _rb_line_y)], fill=ACCENT_CYAN, width=2)

    _rb_score_str = str(round(_rb_score, 1)) if _rb_score is not None else "--"
    _rb_grade_str = str(_rb_grade).upper() if _rb_grade else ""
    draw_text_with_fallback(
        draw, (_rc_x + 160, _rb_top + 60), _rb_score_str,
        fill=_rb_eval_color, font=iosevka_font_28, anchor="mm",
    )
    draw_text_with_fallback(
        draw, (_rc_x + 260, _rb_top + 60), _rb_grade_str,
        fill=_rb_eval_color, font=misans_font_18, anchor='rm',
    )

    # ============================================================
    # Left column: illustration + chains + name + score  (x 40-500)
    # ============================================================

    # --- 1. Character illustration (center ~270, ~550) ---
    _pile_loaded = False
    if role_detail is not None:
        try:
            _role_id = getattr(getattr(role_detail, 'role', None), 'roleId', None)
            if _role_id:
                _is_custom, _pile_raw, _pile_path = await get_role_pile_with_path(_role_id, True)
                _pile_img = resize_and_center_image(_pile_raw, is_custom=_is_custom)
                _pile_w, _pile_h = 460, 720
                _pile_box = Image.new('RGBA', (_pile_w, _pile_h), (0, 0, 0, 0))
                _px = (_pile_w - _pile_img.size[0]) // 2
                _py = (_pile_h - _pile_img.size[1]) // 2
                _pile_box.paste(_pile_img, (_px, _py), _pile_img)
                _cm_path = TEXT_PATH / 'char_mask.png'
                if _cm_path.is_file():
                    _cm = Image.open(_cm_path).convert('RGBA')
                    _cm = _cm.resize((_pile_w, _pile_h), Image.Resampling.LANCZOS)
                    img.alpha_composite(_pile_box, dest=(40, 140))
                    img.alpha_composite(_cm, dest=(40, 140))
                else:
                    img.alpha_composite(_pile_box, dest=(40, 140))
                _pile_loaded = True
        except Exception:
    ...
    if not _pile_loaded:
        _ph_xy = (40, 140, 500, 860)
        try:
            draw.rounded_rectangle(_ph_xy, radius=12, fill=(30, 34, 44, 160), outline=(51, 65, 85, 100), width=1)
        except AttributeError:
            draw.rectangle(_ph_xy, fill=(30, 34, 44, 160), outline=(51, 65, 85, 100), width=1)
        draw_text_with_fallback(
            draw, (270, 500), '\u7acb\u7ed8',
            fill=(100, 110, 130, 140), font=misans_font_32, anchor='mm',
        )

    # --- bottom gradient fade (illustration -> background) ---
    _fade_y0, _fade_h = 740, 140
    for _fi in range(_fade_h):
        _alpha = int(220 * (_fi / _fade_h))
        _fy = _fade_y0 + _fi
        if 0 <= _fy < H:
            draw.line([(40, _fy), (500, _fy)], fill=(*BG_MAIN_V2[:3], _alpha))

    # --- 2. Chains: 6 nodes along left edge ---
    _chain_cx = 68
    _chain_y0 = 240
    _chain_gap = 72
    _chain_r = 28
    _chain_data = []
    if role_detail is not None:
        _chain_data = getattr(role_detail, 'chainList', None) or []

    _attr_col = ACCENT_SILVER
    if role_detail is not None:
        _an = getattr(getattr(role_detail, 'role', None), 'attributeName', None)
        if _an and _an in WAVES_SHUXING_MAP:
            _attr_col = (*WAVES_SHUXING_MAP[_an], 255)

    for _ci in range(6):
        _cy = _chain_y0 + _ci * _chain_gap
        _unlocked = _ci < len(_chain_data) and getattr(_chain_data[_ci], 'unlocked', False)
        _node_ok = False
        _mz_path = TEXT_PATH / 'mz_bg.png'
        if _mz_path.is_file():
            try:
                _mz = Image.open(_mz_path).convert('RGBA')
                if _unlocked:
                    _overlay = Image.new('RGBA', _mz.size, _attr_col)
                    _mz = Image.alpha_composite(_mz, _overlay)
                else:
                    _mz = ImageEnhance.Brightness(_mz).enhance(0.25)
                _mz = _mz.resize((56, 56), Image.Resampling.LANCZOS)
                img.alpha_composite(_mz, dest=(_chain_cx - _chain_r, _cy - _chain_r))
                _node_ok = True
            except Exception:
                _node_ok = False

        if not _node_ok:
            _node_fill = _attr_col if _unlocked else (50, 55, 65, 160)
            draw.ellipse(
                (_chain_cx - _chain_r, _cy - _chain_r, _chain_cx + _chain_r, _cy + _chain_r),
                fill=_node_fill, outline=(51, 65, 85, 100), width=1,
            )

        draw_text_with_fallback(
            draw, (_chain_cx, _cy), str(_ci + 1),
            fill=(200, 210, 220, 200) if _unlocked else (80, 90, 100, 140),
            font=iosevka_font_20, anchor='mm',
        )

    # --- 3. Big name plate (y=900) ---
    _name_y = 900
    _name_x = 270
    draw_text_with_fallback(
        draw, (_name_x, _name_y), char_name,
        fill=ACCENT_SILVER, font=youshe_font_32, anchor='mm',
    )

    # --- 4. Score slot (y=1040) ---
    _sc_y = 1040
    _sc_x = 270
    _sc_score_str = str(round(_rb_score, 1)) if _rb_score is not None else "--"
    _sc_grade_str = str(_rb_grade).upper() if _rb_grade else ""
    draw_text_with_fallback(
        draw, (_sc_x, _sc_y), _sc_score_str,
        fill=_rb_eval_color, font=iosevka_font_28, anchor='mm',
    )
    draw_text_with_fallback(
        draw, (_sc_x + 60, _sc_y), _sc_grade_str,
        fill=_rb_eval_color, font=misans_font_20, anchor='lm',
    )

    # ============================================================
    # Bottom: chain detail card (full width, below illustration)
    # ============================================================
    _cr_x = 40
    _cr_w = 460
    _cr_card_y = 1120
    ...
    _cr_rows = []
    for _ci in range(6):
        if _ci < len(_chain_data):
            _cd = _chain_data[_ci]
            _cr_name = getattr(_cd, 'name', None) or ""
            _cr_desc = getattr(_cd, 'description', None) or ""
            _cr_unlocked = getattr(_cd, 'unlocked', False)
        else:
            _cr_name = ""
            _cr_desc = ""
            _cr_unlocked = False
        _cr_rows.append((_cr_name, _cr_desc, _cr_unlocked))

    _cr_row_h = 48
    _cr_title_area = 48
    _cr_card_h = _cr_title_area + len(_cr_rows) * _cr_row_h + 20

    _cr_card_xy = (_cr_x, _cr_card_y, _cr_x + _cr_w, _cr_card_y + _cr_card_h)
    try:
        draw.rounded_rectangle(_cr_card_xy, radius=12, fill=BG_CARD_V2, outline=(51, 65, 85), width=1)
    except AttributeError:
        draw.rectangle(_cr_card_xy, fill=BG_CARD_V2, outline=(51, 65, 85), width=1)

    _cr_title_x = _cr_x + 16
    _cr_title_y = _cr_card_y + 24
    draw_text_with_fallback(
        draw, (_cr_title_x, _cr_title_y), "\u5171\u9e23\u94fe",
        fill=ACCENT_SILVER, font=misans_font_20, anchor="lm",
    )
    _cr_line_y = _cr_title_y + 14
    draw.line([(_cr_title_x, _cr_line_y), (_cr_title_x + 28, _cr_line_y)], fill=ACCENT_CYAN, width=2)

    for _ri, (_cr_name, _cr_desc, _cr_unlocked) in enumerate(_cr_rows):
        _cry = _cr_card_y + _cr_title_area + _ri * _cr_row_h + 20
        _cr_state_col = ACCENT_SILVER if _cr_unlocked else (80, 90, 100, 140)
        _cr_idx_str = str(_ri + 1)
        draw_text_with_fallback(
            draw, (_cr_x + 16, _cry), _cr_idx_str + ".",
            fill=_cr_state_col, font=iosevka_font_20, anchor="lm",
        )
        draw_text_with_fallback(
            draw, (_cr_x + 40, _cry), _cr_name or "---",
            fill=_cr_state_col, font=misans_font_18, anchor="lm",
        )

    # ============================================================
    # Right column: 5 phantom cards (\u7cbe\u7ec6\u7248: \u56fe\u6807\u5360\u4f4d+\u540d\u79f0+\u7b49\u7ea7+\u8bc4\u5206+\u4e3b\u526f\u8bcd\u6761+\u597d\u8bcd\u6761\u9ad8\u4eae)
    # ============================================================
    _disc_r_top = _rb_top + _rb_height + 16

    _phantoms = []
    if role_detail is not None:
        pd = getattr(role_detail, "phantomData", None)
        if pd:
            _phantoms = getattr(pd, "equipPhantomList", None) or []
    ...
    if role_detail is not None:
        try:
            from ..utils.damage.modal import get_role_modal
            _cm_obj = get_calc_map({}, role_detail.role.roleName, role_detail.role.roleId, get_role_modal(role_detail))
            _v2_calc_map = getattr(_cm_obj, "calc_temp", None) or _cm_obj
        except Exception:
            _v2_calc_map = None

    _pc_label_x = _rc_x + 16          # 862
    _pc_val_rx = _rc_x + 298          # 1144
    _pc_sub_c1_x = _rc_x + 16         # \u526f\u8bcd\u6761\u7b2c1\u5217 \u540d
    _pc_sub_c1_rx = _rc_x + 152       # \u526f\u8bcd\u6761\u7b2c1\u5217 \u503c\u53f3\u5bf9\u9f50
    _pc_sub_c2_x = _rc_x + 164        # \u526f\u8bcd\u6761\u7b2c2\u5217 \u540d
    _pc_sub_c2_rx = _rc_x + 298       # \u526f\u8bcd\u6761\u7b2c2\u5217 \u503c\u53f3\u5bf9\u9f50
    _pc_card_h = 150
    _pc_gap = 14
    _pc_y = _disc_r_top
    _last_pc_bottom = _disc_r_top

    for _pi in range(5):
        _ph = _phantoms[_pi] if _pi < len(_phantoms) else None
        _pc_top = _disc_r_top + _pi * (_pc_card_h + _pc_gap)
        _pc_bottom = _pc_top + _pc_card_h
        _last_pc_bottom = _pc_bottom
        try:
            draw.rounded_rectangle((_rc_x, _pc_top, _rc_x + _rc_w, _pc_bottom), radius=12, fill=BG_CARD_V2, outline=(51, 65, 85), width=1)
        except AttributeError:
            draw.rectangle((_rc_x, _pc_top, _rc_x + _rc_w, _pc_bottom), fill=BG_CARD_V2, outline=(51, 65, 85), width=1)

        if _ph is None or getattr(_ph, "phantomProp", None) is None:
            draw_text_with_fallback(draw, (_pc_label_x, _pc_top + 18), "\u672a\u88c5\u5907", fill=TEXT_SUB, font=misans_font_16, anchor="lm")
            continue

        _ph_prop = _ph.phantomProp
        _ph_name = getattr(_ph_prop, "name", None) or "---"
        _ph_name = str(_ph_name).replace("\u00b7", " ")
        _ph_lv = getattr(_ph, "level", None)
        _ph_cost = getattr(_ph, "cost", None)
        try:
            _ph_props = _ph.get_props()
        except Exception:
            _ph_props = (getattr(_ph, "mainProps", None) or []) + (getattr(_ph, "subProps", None) or [])
        try:
            _ph_score, _ = calc_phantom_score(role_detail.role.roleId, _ph_props, _ph_cost, _v2_calc_map)
        except Exception:
            _ph_score = None

        # \u9876\u90e8\u884c: \u56fe\u6807\u5360\u4f4d + \u540d\u79f0 + (\u53f3)\u7b49\u7ea7\u00b7\u8bc4\u5206
        draw.rectangle((_pc_label_x, _pc_top + 12, _pc_label_x + 28, _pc_top + 40), fill=(40, 44, 54, 160), outline=(51, 65, 85, 100), width=1)
        draw_text_with_fallback(draw, (_pc_label_x + 38, _pc_top + 26), _ph_name, fill=TEXT_MAIN, font=misans_font_18, anchor="lm")
        _hdr_r = ("Lv." + str(_ph_lv) if _ph_lv is not None else "Lv.--") + ("  " + str(round(_ph_score, 1)) + "\u5206" if _ph_score is not None else "")
        draw_text_with_fallback(draw, (_pc_val_rx, _pc_top + 26), _hdr_r, fill=ACCENT_SILVER, font=iosevka_font_18, anchor="rm")

        # \u4e3b\u8bcd\u6761(\u524d2\u4e2a)
        _main_props = _ph_props[:2]
        _sub_props = _ph_props[2:]
        _my = _pc_top + 56
        for _mi, _mp in enumerate(_main_props):
            _mn_c, _mv_c = get_valid_color(_mp.attributeName, _mp.attributeValue, _v2_calc_map)
            draw_text_with_fallback(draw, (_pc_label_x, _my + _mi * 24), str(_mp.attributeName).replace("\u767e\u5206\u6bd4", "%"), fill=_mn_c, font=misans_font_16, anchor="lm")
            draw_text_with_fallback(draw, (_pc_val_rx, _my + _mi * 24), str(_mp.attributeValue), fill=_mv_c, font=iosevka_font_18, anchor="rm")

        # \u526f\u8bcd\u6761(\u53cc\u5217: \u5de63\u53f32)
        _sy0 = _pc_top + 104
        _sub_row_h = 22
        for _si, _sp in enumerate(_sub_props[:5]):
            _col = 0 if _si < 3 else 1
            _row = _si if _si < 3 else _si - 3
            _nx = _pc_sub_c1_x if _col == 0 else _pc_sub_c2_x
            _vx = _pc_sub_c1_rx if _col == 0 else _pc_sub_c2_rx
            _sy = _sy0 + _row * _sub_row_h
            _sn_c, _sv_c = get_valid_color(_sp.attributeName, _sp.attributeValue, _v2_calc_map)
            draw_text_with_fallback(draw, (_nx, _sy), str(_sp.attributeName).replace("\u767e\u5206\u6bd4", "%"), fill=_sn_c, font=misans_font_14, anchor="lm")
            draw_text_with_fallback(draw, (_vx, _sy), str(_sp.attributeValue), fill=_sv_c, font=misans_font_14, anchor="rm")

    _disc_height = _last_pc_bottom - _disc_r_top
    _disc_r_title_y = _disc_r_top + 28

    # ============================================================
    # Right column: ratio/weight summary card
    # ============================================================
    _ratio_mc_top = _disc_r_top + _disc_height + 16
    _ratio_mc_title_y = _ratio_mc_top + 28
    ...
    _ratio_row_h = 32
    _ratio_row_start = _ratio_mc_top + 60
    _ratio_mc_card_h = 60 + len(_ratio_rows) * _ratio_row_h + _card_bottom_pad

    _ratio_mc_xy = (_rc_x, _ratio_mc_top, _rc_x + _rc_w, _ratio_mc_top + _ratio_mc_card_h)
    try:
        draw.rounded_rectangle(_ratio_mc_xy, radius=12, fill=BG_CARD_V2, outline=(51, 65, 85), width=1)
    except AttributeError:
        draw.rectangle(_ratio_mc_xy, fill=BG_CARD_V2, outline=(51, 65, 85), width=1)

    _ratio_mc_title_x = _rc_x + 16
    draw_text_with_fallback(
        draw, (_ratio_mc_title_x, _ratio_mc_title_y), "\u6c47\u603b",
        fill=ACCENT_SILVER, font=misans_font_20, anchor="lm",
    )
    _ratio_mc_line_y = _ratio_mc_title_y + 14
    draw.line([(_ratio_mc_title_x, _ratio_mc_line_y), (_ratio_mc_title_x + 28, _ratio_mc_line_y)], fill=ACCENT_CYAN, width=2)

    for _ri2, (_r_name, _r_val) in enumerate(_ratio_rows):
        _rry = _ratio_row_start + _ri2 * _ratio_row_h
        draw_text_with_fallback(
            draw, (_rc_x + 16, _rry), _r_name,
            fill=TEXT_SUB, font=misans_font_18, anchor="lm",
        )
        draw_text_with_fallback(
            draw, (_rc_x + 298, _rry), _r_val,
            fill=ACCENT_SILVER, font=iosevka_font_18, anchor="rm",
        )

    # ============================================================
    # Bottom: relic score summary (left column, below chain card)
    # ============================================================
    _rc_sum_top = _cr_card_y + _cr_card_h + 16
    _rc_sum_title_y = _rc_sum_top + 28

    _phantom_scores = []
    try:
        for _ph in _phantoms:
            _sc, _gr = calc_phantom_score(_ph)
            _phantom_scores.append((_sc, _gr))
    except Exception:
        _phantom_scores = [(None, None)] * len(_phantoms)

    _sum_row_h = 32
    _sum_row_start = _rc_sum_top + 60
    _sum_card_bottom = 24
    _sum_height = 60 + len(_phantoms) * _sum_row_h + _sum_card_bottom

    _sum_xy = (_cr_x, _rc_sum_top, _cr_x + _cr_w, _rc_sum_top + _sum_height)
    _rc_border = (51, 65, 85)
    try:
        draw.rounded_rectangle(_sum_xy, radius=12, fill=BG_CARD_V2, outline=_rc_border, width=1)
    except AttributeError:
        draw.rectangle(_sum_xy, fill=BG_CARD_V2, outline=_rc_border, width=1)

    _rc_title_x = _cr_x + 16
    _sum_title_y_real = _rc_sum_top + 24
    draw_text_with_fallback(
        draw, (_rc_title_x, _sum_title_y_real), "\u58f0\u9abf\u8bc4\u5206",
        fill=ACCENT_SILVER, font=misans_font_20, anchor="lm",
    )
    _sum_line_y = _sum_title_y_real + 14
    draw.line([(_rc_title_x, _sum_line_y), (_rc_title_x + 28, _sum_line_y)], fill=ACCENT_CYAN, width=2)

    for _pi, (_ps, _pg) in enumerate(_phantom_scores):
        _pry = _sum_row_start + _pi * _sum_row_h
        _ph_prop2 = getattr(_phantoms[_pi], "phantomProp", None) if _pi < len(_phantoms) else None
        _ph_nm = getattr(_ph_prop2, "name", None) or "---" if _ph_prop2 else "---"

        draw_text_with_fallback(
            draw, (_cr_x + 16, _pry), _ph_nm,
            fill=TEXT_SUB, font=misans_font_18, anchor="lm",
        )
        _ps_str = str(round(_ps, 1)) if _ps is not None else "--"
        _pg_str = str(_pg).upper() if _pg else ""
        draw_text_with_fallback(
            draw, (_cr_x + 340, _pry), _ps_str,
            fill=ACCENT_SILVER, font=iosevka_font_18, anchor="rm",
        )
        draw_text_with_fallback(
            draw, (_cr_x + 380, _pry), _pg_str,
            fill=ACCENT_CYAN, font=misans_font_18, anchor='lm',
        )

    # --- version watermark ---
    draw_text_with_fallback(
        draw, (W - 40, H - 20), "EchoMatrix v2",
        fill=(100, 110, 130, 80), font=misans_font_14, anchor='rm',
    )

    # Recalculate canvas height based on all three columns
    _left_bottom = _cr_card_y + _cr_card_h
    _mid_bottom = _ratio_mc_top + _ratio_mc_card_h
    _right_bottom = _disc_r_top + 36
    _content_bottom = max(_left_bottom, _mid_bottom, _right_bottom) + 40
    if _content_bottom > H:
        _new_img = Image.new("RGBA", (W, _content_bottom), BG_MAIN_V2)
        _new_img.alpha_composite(img, dest=(0, 0))
        img = _new_img
        draw = ImageDraw.Draw(img)
        H = _content_bottom
    # --- convert & return ---
